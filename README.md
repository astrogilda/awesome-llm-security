...
- [agent-evidence-vectors](https://github.com/astrogilda/agent-evidence-vectors): conformance vectors and a reference verifier for the in-toto adversarial-execution-evidence predicate; the consumer recomputes the outcome from the bytes an attestation carries rather than trusting the producer's verdict.
